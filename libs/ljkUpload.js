    ...
    LjkUpload.prototype = {
        ...
        readerImage: function (fileType, file, maxSize, callback) {
            ...
            if (fileType == _this.fileTypeConfig['img']) {
                ...
                if (!reg.test(file.type)) {
                    _this.errorNotice("不支持" + !!type ? type : '这种' + "格式的" + fileType + "哟");
                    return;
                }
            }
        ...
        multipleUpload: function (options) {
            ...
            _this.getFileInfo(options.fileBtn, options.maxSize, _this.fileTypeConfig['img'], true, function (data) {
                ...
                _this.multipleSizeChange(options.onChange, data, size, 1, true);
                _this.removeMultipleItem(showEle.remove, function () {
                    _this.multipleSizeChange(options.onChange, data, size, _this.multipleNum, false)
                });
                _this.appendImage(data.result, options.fileSelectBtn, showEle.warp);
                options.fileBtn.blur();
            })

            //绑定粘贴事件
            options.paste === true && this.addPasteListener(function (file) {
                var showEle = _this.createMultipleItem(++id, file.size);
                multipleSection.append(showEle.item);
                _this.readerImage(_this.fileTypeConfig['img'], file, options.maxSize, function (data) {
                    _this.appendImage(data.result, options.fileSelectBtn, showEle.warp);
                });
            ...
            if (options.drag === true && options.dragArea) {
                _this.addListener(
                    $.extend(
                        ...
                        {
                            zoom: false,
                            type: _this.fileTypeConfig['img'],
                            callback: function (data) {
                                var showEle = _this.createMultipleItem(++id, data.size);
                                multipleSection.append(showEle.item);
                                _this.appendImage(data.result, options.fileSelectBtn, showEle.warp)
                            }
                        }
        ...
        createMultipleItem: function (id, size) {
            var item = $('<li class="multiple-item" id="upload-' + id + '"><div class="mask"><a href="javascript:;" data-id="' + id + '" data-size="' + size || 0 + '">删除</a></div><div class="item-warp hasImg"></div></li>')
            return {
                item: item,
